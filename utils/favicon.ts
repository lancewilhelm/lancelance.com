...
/**
 * Function to generate the favicon SVG. Returns the data as a string
 */
export function generateFaviconSVG(theme: string) {
  const colors = themesList.find((t) => t.name === theme) || themesList[0]
  const mainColor = colors.mainColor
  const bgColor = colors.bgColor
  ...
  const svgString = `
    <svg width="100%" height="100%" viewBox="0 0 256 256" version="1.1" xmlns="http://www.w3.org/2000/svg">
        <style>
          #bg { fill: none; stroke: ${mainColor}; stroke-width: 7.49px; }
          #fg { fill: none; stroke: ${bgColor}; stroke-width: 2.76px; }
        </style>
        <g transform="matrix(1,0,0,1,0,-134.314)">
            <g id="favicon" transform="matrix(0.2,0,0,0.32,0,134.314)">
                <g transform="matrix(1.09868,0,0,1.10761,-32.0919,-3.59777)">
